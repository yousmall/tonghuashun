...
from sqlalchemy import DateTime, ForeignKey, Integer, JSON, String, Text, create_engine, func, select
    ...
    def list_conversations(self, user_id: int, limit: int = 50) -> list[dict[str, Any]]:
        ...
        with self.session_factory() as session:
            rows = session.execute(
                select(ConversationRow, count_query.c.message_count)
            ...
            for conversation, message_count in rows:
                last_message = session.scalar(
                    select(MessageRow.content).where(MessageRow.conversation_id == conversation.id)
